...
public class HealthDatabaseHandler extends SQLiteOpenHelper {
	...
	private final ArrayList<HeartBeats> heartBeatsList = new ArrayList<HeartBeats>();
	private final ArrayList<BloodPressures> bloodPressuresList = new ArrayList<BloodPressures>();
	...
	public void onCreate(SQLiteDatabase db) {
		String CREATE_ACTIVITY_TABLE = "CREATE TABLE " + ACTIVITY_TABLE + "("
				+ KEY_TIMESTAMP + " TEXT PRIMARY KEY," + DISTANCE + " INTEGER,"
				+ DURATION + " REAL," + STEPS + " INTEGER" + ")";

		String CREATE_SLEEP_TABLE = "CREATE TABLE " + SLEEP_TABLE + "("
				+ KEY_TIMESTAMP + " TEXT PRIMARY KEY," + SLEEP_EFFICIENCY
				+ " INTEGER," + SLEEP_START + " TEXT," + SLEEP_MINS
				+ " INTEGER," + AWAKE_MINS + " INTEGER," + AWAKE_COUNT
				+ " INTEGER," + TOTAL_SLEEP_TIME + " INTEGER" + ")";

		String CREATE_HEART_RATE_TABLE = "CREATE TABLE " + HEART_RATE_TABLE
				+ "(" + KEY_TIMESTAMP + " TEXT PRIMARY KEY," + HEART_RATE
				+ " INTEGER" + ")";

		String CREATE_BLOOD_PRESSURE_TABLE = "CREATE TABLE "
				+ BLOOD_PRESSURE_TABLE + "(" + KEY_TIMESTAMP
				+ " TEXT PRIMARY KEY," + SYSTOLIC + " INTEGER," + DIASTOLIC
				+ " INTEGER" + ")";
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					activities
							.setDistance(Integer.parseInt(cursor.getString(1)));
					activities.setDuration(Double.parseDouble(cursor
							.getString(2)));
		...
		try {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					sleep.setEfficiency(Integer.parseInt(cursor.getString(1)));
					sleep.setMinutesAsleep(Integer.parseInt(cursor.getString(3)));
					sleep.setMinutesAwake(Integer.parseInt(cursor.getString(4)));
					sleep.setAwakeningsCount(Integer.parseInt(cursor
							.getString(5)));
					sleep.setTimeInBed(Integer.parseInt(cursor.getString(6)));
	...
	public ArrayList<HeartBeats> getHeartBeats() {
		...
		try {
			...
			// Select All Query
			String selectQuery = "SELECT  * FROM " + HEART_RATE_TABLE;
			...
			if (cursor.moveToFirst()) {
				do {
					...
					heartBeats.setDate(timeStamp);
					heartBeats.setTime(timeStamp);
					heartBeats.setCount(Integer.parseInt(cursor.getString(1)));

					// Adding heartbeats to list
					heartBeatsList.add(heartBeats);
				} while (cursor.moveToNext());
			}
			...
			Log.e("bloodPressuresList", "" + e);
		}

		return heartBeatsList;
	}

	public ArrayList<BloodPressures> getBloodPressures() {

		try {
			bloodPressuresList.clear();

			// Select All Query
			String selectQuery = "SELECT  * FROM " + BLOOD_PRESSURE_TABLE;
			...
			if (cursor.moveToFirst()) {
				do {
					...
					bloodPressures.setDate(timeStamp);
					bloodPressures.setTime(timeStamp);
					bloodPressures.setSystolic(Integer.parseInt(cursor
							.getString(1)));
					bloodPressures.setDiastolic(Integer.parseInt(cursor
							.getString(2)));

					// Adding bloodPressures to list
					bloodPressuresList.add(bloodPressures);
				} while (cursor.moveToNext());
			}
